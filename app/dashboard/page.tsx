...
import {Card, Flex, Heading, IconButton, Inset, Separator, Text} from "@radix-ui/themes";
...
export default function Dashboard() {
    ...
    return (
        <Flex gap={"2"} direction={"column"} maxWidth={"500px"} mx={"auto"} my={"3"} justify={"center"}>
            <Inset side={"top"}>
                <Flex justify={"between"} align={"center"}>
                    <Heading size={"5"} className={"title"}>Dashboard</Heading>
                    <IconButton onClick={
                        async () => {
            ...
            {loading ? (
                ...
                <Flex direction={"column"} gap={"2"} m={"2"}>
                    {taskLists.map((taskList) => (
                        <Card key={taskList.id} size={"1"} onClick={
                            () => {
